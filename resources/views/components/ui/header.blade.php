  ...
  <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
    ...
    <div class="hidden lg:flex lg:gap-x-12">
        @foreach($links as $link)
            <a href="{{ $link['url'] }}" class="text-sm font-semibold leading-6 text-gray-900">{{ $link['label'] }}</a>
        @endforeach
    </div>
      ...
      <div class="mt-6 flow-root">
        <div class="-my-6 divide-y divide-gray-500/10">
          <div class="space-y-2 py-6">
              @foreach($links as $link)
                  <a href="{{ $link['url'] }}" class="-m-3 block rounded-lg py-2 px-3 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">{{ $link['label'] }}</a>
              @endforeach
          </div>
          <div class="py-6">
